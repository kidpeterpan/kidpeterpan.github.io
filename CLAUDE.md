...
**Ordered course sections**: a section becomes a sequential course — listed oldest-first, with previous/next episode links on each post — by setting `ordered = true` under `[params]` in its `_index.md`. `go/` and `git/` use this; `article/`, `book/`, and `cheat_sheet/` do not and stay newest-first, as do all `/tags/*` pages (taxonomy terms have no `_index.md`, so they never carry the flag). Episode order comes from the two-digit filename prefix (`01-`…`11-`), never from `date` — Go episodes 07 and 08 share a date, and Hugo's `.NextInSection`/`.PrevInSection` order by date and point the wrong way across that tie. Add a new episode by continuing the numbering; nothing else is required. `layouts/partials/ordered-pages.html` is the single source of that ordering — both `list.html` and `episode-nav.html` must resolve pages through it, or navigation could link to a page the listing omits.
